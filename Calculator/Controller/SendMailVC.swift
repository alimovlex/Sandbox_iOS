...
class SendMailVC: UITableViewController, UITextFieldDelegate {
    ...
    func updateUI() {
        ...
        fromEmailTextField.text = fromEmail;
        subjectMailTextField.text = emailSubject;
        bccEmailTextField.text = ccEmail;
        ccEmailTextField.text = ccEmail;
        mailContentsTextField.text.append(signature);
    }
    
    func createMessage() -> MCOMessageBuilder {
        let builder = MCOMessageBuilder();
        
        if let receiversAddress = MCOAddress(mailbox: fromEmailTextField.text) {
            builder.header.from = MCOAddress(displayName: self.fromName, mailbox: toEmailTextField.text);
    ...
    */
    ...
    @IBAction func sendEmailBtnPressed(_ sender: Any) {
         ...
         */
        sendSMTPMessage(with: session, and: messageBuilder);
